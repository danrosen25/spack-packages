...
def generate_esmfconfig(esmfmkfile, template, outputdir):
    """Generate ESMF CMake package configuration files from `esmf.mk`."""

    if not os.path.exists(esmfmkfile):
        print(f"Error: Specified esmf.mk file does not exist at: {esmfmkfile}", file=sys.stderr)
        sys.exit(1)
    ...
    libs_dir = esmf_vars.get("ESMF_LIBSDIR", os.path.dirname(esmfmkfile))
    lib_loc = os.path.join(libs_dir, "libesmf.so")
    if not os.path.exists(lib_loc):
        lib_loc = os.path.join(libs_dir, "libesmf.a")

    if not os.path.exists(template):
        print(f"Error: Template file not found at {template}", file=sys.stderr)
        sys.exit(1)
    ...
    # Outputdir
    out_dir = os.path.abspath(outputdir)
    ...
    version_file_content = f"""# Generated automatically by generate_esmfconfig.py - Do not edit
